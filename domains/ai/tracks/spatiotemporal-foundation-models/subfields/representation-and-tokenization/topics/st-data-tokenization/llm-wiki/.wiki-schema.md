...
```
/Users/chizhang/Desktop/career/plan/domains/ai/tracks/spatiotemporal-foundation-models/subfields/representation-and-tokenization/topics/st-data-tokenization/llm-wiki/
├── raw/                    # 原始素材（AI 只读，不会修改）
│   ├── articles/           # 网页文章
│   ├── tweets/             # X/Twitter 内容
│   ├── wechat/             # 微信公众号文章
...
- 辩论记录：`wiki/debates/{辩论主题}.md`
  - 例：`wiki/debates/ST-token 与 NLP-CV token 的差异和影响面.md`
...
1. 当讨论涉及研究路线、概念定义、技术取舍、方法优劣或影响范围时，保存到 `wiki/debates/`。
2. 辩论记录必须在 frontmatter 中标记 `type: debate` 和 `derived: true`。
3. 辩论记录不是一手素材；后续引用其结论时，应标记为观点或推断，而不是论文事实。
4. 每条辩论记录应包含：辩论问题、核心判断、关键分歧、待验证问题和相关页面。
5. 文件名直接使用辩论主题名称；如果主题包含 `/` 等路径保留字符，用 `-` 替代。
